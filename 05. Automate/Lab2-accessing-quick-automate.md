...
이전 섹션에서 `sample-kyc-documents.zip`을 다운로드하고 압축 해제했습니다. 압축 해제된 `sample-kyc-documents/12345/` 폴더 안에 고객 ID 12345에 대한 두 개의 샘플 고객 문서가 있습니다:

- `Janice Sample-DriverLicense.jpg` — 운전면허증
- `Janice Sample-StatementofAccount.pdf` — 은행 명세서

아직 번들을 다운로드하지 않았다면 [워크샵 자료 다운로드](./README.md) 페이지에서 받고 여기로 돌아오세요.
